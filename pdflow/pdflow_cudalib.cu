 ...
 * Changes:
 * 
 * 1. Widths and heights are changed for Kinect V2
 *
 */
...
{
    const unsigned int width = 512/cam_mode;
    const unsigned int height = 424/cam_mode;
    unsigned int s;
...
void GaussianPyramidBridge(CSF_cuda *csf, unsigned int levels, unsigned int cam_mode)
{
    ...
    {
        const unsigned int cols_i_aux = 512/(cam_mode*powf(2,i));
        const unsigned int rows_i_aux = 424/(cam_mode*powf(2,i));
